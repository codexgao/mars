...
// Build: g++ -std=c++17 xlog_native_test.cpp -o xlog_native_test -L. -lxlog -Wl,-rpath,.
// Run:   ./xlog_native_test (libxlog.so must be discoverable)
...
static std::string getTempDir() {
  ...
  if (!base) base = "/tmp";
  std::string dir = std::string(base) + "/xlog_linux_test/";
...
int main() {
  std::cout << "=== xlog Linux C API Tests ===" << std::endl;
  std::cout << "(TODO: verify on Linux target platform)" << std::endl;
...
#else
#error "This test is for Linux platform only (__linux__ must be defined, _WIN32 must not)"
#endif
