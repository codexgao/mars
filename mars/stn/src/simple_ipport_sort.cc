...
static inline uint32_t CAL_LAST_CONTINUOUS_BIT_COUNT(uint64_t RECORDS) {
    uint32_t COUNT = 0;
...
void SimpleIPPortSort::__RemoveTimeoutXml() {
    ...
    for (tinyxml2::XMLElement* record = recordsxml_.FirstChildElement(kRecord); NULL != record;
         record = record->NextSiblingElement(kRecord)) {
...
void SimpleIPPortSort::InitHistory2BannedList(bool _savexml) {
    ...
    if (kNoNet == getCurrNetLabel(curr_netinfo))
        return;

    const tinyxml2::XMLElement* record = NULL;

    for (record = recordsxml_.FirstChildElement(kRecord); NULL != record;
         record = record->NextSiblingElement(kRecord)) {
    ...
    for (const tinyxml2::XMLElement* item = record->FirstChildElement(kItem); NULL != item;
         item = item->NextSiblingElement(kItem)) {
...
void SimpleIPPortSort::Update(const std::string& _ip, uint16_t _port, bool _is_success) {
    ...
    if (kNoNet == getCurrNetLabel(curr_net_info))
        return;

    ScopedLock lock(mutex_);

    if (!__CanUpdate(_ip, _port, _is_success))
        return;
    ...
    for (record = recordsxml_.FirstChildElement(kRecord); NULL != record;
        ...
        if (netinfo_chr && (0 == strcmp(netinfo_chr, curr_net_info.c_str())))
            break;
    }
    ...
    for (item = record->FirstChildElement(kItem); NULL != item; item = item->NextSiblingElement(kItem)) {
        const char* ip = item->Attribute(kIP);
        uint32_t port = item->UnsignedAttribute(kPort);

        if (ip && (0 == strcmp(ip, _ip.c_str())) && port == _port)
            break;
    }
...
void SimpleIPPortSort::__SortbyBanned(std::vector<IPPortItem>& _items, bool _use_IPv6) const {
    ...
    auto find_lambda = [&](const IPPortItem& _v) {
        for (std::vector<BanItem>::const_iterator it_banned = _ban_fail_list_.begin();
             it_banned != _ban_fail_list_.end();
             ++it_banned) {
    ...
    // sort history
    std::sort(items_history.begin(), items_history.end(), [&](const IPPortItem& _l, const IPPortItem& _r) {
...
void SimpleIPPortSort::__PickIpItemRandom(std::vector<IPPortItem>& _items,
...
void SimpleIPPortSort::SortandFilter(std::vector<IPPortItem>& _items, int _needcount, bool _use_IPv6) const {
    ...
    if (_needcount < (int)_items.size())
        _items.resize(_needcount);
}
